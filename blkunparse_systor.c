...
void get_rwbs(struct blk_io_trace* bio_, char* tok[]){
    char *rwbs = tok[2];//CHANGE FOR systor
    //printf("len = %lu\n", strlen(rwbs));
    //printf("rwbs_str = %s\n", rwbs);
...
struct blk_io_trace get_bit(char * tok[]){
    ...
    int i=0;
    char *time_arr[2];
    time_parts = strtok(tok[0], delim);//CHANGE FOR systor
    ...
    int cpu = sequence%8;//CHANGE FOR systor
    int pid = 1234;//CHANGE FOR systor
    ...
    get_device_code(&bio_, tok[3]);//CHANGE FOR systor
